...
export class NotificationsService {
  ...
  public getGameInvites(inviteeId: number): GameInvite[] {
    return this.gameService.games
      .map((game) =>
        game.invites.filter((invite) => invite.invitee.id === inviteeId)
      )
      .flat()
      .filter(Boolean);
  }
  ...
  public getGameInvite(inviteeId: number, game?: Game): GameInvite | null {
    if (game) {
      return (
        game.invites.find((invite) => invite.invitee.id === inviteeId) || null
      );
    }

    for (const game of this.gameService.games) {
      const invite = game.invites.find(
        (invite) => invite.invitee.id === inviteeId
      );
      if (invite) {
        return invite;
      }
  ...
  async sendGameInvite(socket: SocketType, userId: number) {
    ...
    if (!game) throw new WsException("Nie jesteś w żadnej grze");

    const invitee = await this.userService.getUserById(userId);
    if (!invitee) throw new WsException("Nie znaleziono użytkownika");
  ...
  private declineGameInvite(
    ...
    if (game) {
      const inviteIndex = game.invites.findIndex(
  ...
  private sendRemoveNotification(
    socket: SocketType,
    notification: INotification
  ) {
  ...
  onJoinGame(socket: SocketType, game: Game) {
    ...
    const inviteIndex = game.invites.findIndex(
      (invite) => invite.invitee.id === socket.data.user.id
    );
    game.invites.splice(inviteIndex, 1);
