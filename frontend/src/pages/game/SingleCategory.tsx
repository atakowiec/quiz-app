...
export default function SingleCategory({categoryId}: {categoryId: number}) {
  ...
  const game = useGame();
  const category = useGlobalData().categories.find((category) => category.id === categoryId);
  if (!category) {
    return null;
  }
  ...
  const playersVoted = game?.players?.filter(player => player.chosenCategory === categoryId);

  return (
    <div className={`${styles.choice} ${voted ? styles.selected : ""}`} onClick={voteForCategory}>
      <img src={`/assets/categories/${category.img}`} alt={category.name} />
      {category.name}
